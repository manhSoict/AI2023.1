...
class game:
    ...
    def makehiscorelist(self):
        ...
        for line in hs:
            vpos += 12
            linesurf = f.render(
                line[1].rjust(22) + str(line[0]).rjust(9), 1, (255, 255, 255)
            )
    ...
    def DrawScore(self):
        ...
        screen.blit(
            thisFruit.imFruit[thisFruit.fruitType], (4 + 16, self.screenSize[1] - 20)
        )
        ...
        if self.mode == 3:
            ...
            screen.blit(
                self.imReady, (self.screenSize[0] / 2 - 20, self.screenSize[1] / 2 + 12)
            )
...
class node:
    def __init__(self):
        ...
        self.h = (
            -1
        )  # estimated movement cost to move from this node to the ending node (remaining horizontal and vertical steps * 10)
        ...
        if not self.currentPath == False:
            if len(self.currentPath) > 0:
                ...
                if not self.state == 3:
                    ...
                    while not thisLevel.GetMapTile(randRow, randCol) == tileID[
                        "pellet"
...
class pacman:
    def __init__(self):
        ...
        for i in range(1, 9, 1):
            self.anim_pacmanL[i] = pygame.image.load(
                os.path.join(
                    SCRIPT_PATH, "res", "sprite", "pacman-l " + str(i) + ".gif"
                )
            ).convert()
            self.anim_pacmanR[i] = pygame.image.load(
                os.path.join(
                    SCRIPT_PATH, "res", "sprite", "pacman-r " + str(i) + ".gif"
                )
            ).convert()
            self.anim_pacmanU[i] = pygame.image.load(
                os.path.join(
                    SCRIPT_PATH, "res", "sprite", "pacman-u " + str(i) + ".gif"
                )
            ).convert()
            self.anim_pacmanD[i] = pygame.image.load(
                os.path.join(
                    SCRIPT_PATH, "res", "sprite", "pacman-d " + str(i) + ".gif"
                )
            ).convert()
    ...
    def Move(self):
        ...
        if not thisLevel.CheckIfHitWall(
            self.x + self.velX, self.y + self.velY, self.nearestRow, self.nearestCol
        ):
            # it's ok to Move
            self.x += self.velX
            self.y += self.velY
            ...
            thisLevel.CheckIfHitSomething(
                self.x, self.y, self.nearestRow, self.nearestCol
            )
    ...
    def Draw(self):
        ...
        screen.blit(
            self.anim_pacmanCurrent[self.animFrame],
...
class level:
    ...
    def CheckIfHitWall(self, possiblePlayerX, possiblePlayerY, row, col):
        ...
        for iRow in range(row - 1, row + 2, 1):
            for iCol in range(col - 1, col + 2, 1):
                if (
                    (possiblePlayerX - (iCol * 16) < 16)
                    and (possiblePlayerX - (iCol * 16) > -16)
                    and (possiblePlayerY - (iRow * 16) < 16)
                    and (possiblePlayerY - (iRow * 16) > -16)
                ):
    ...
    def CheckIfHit(self, playerX, playerY, x, y, cushion):
        if (
            (playerX - x < cushion)
            and (playerX - x > -cushion)
            and (playerY - y < cushion)
            and (playerY - y > -cushion)
        ):
            return True
        else:
    ...
    def CheckIfHitSomething(self, playerX, playerY, row, col):
        for iRow in range(row - 1, row + 2, 1):
            for iCol in range(col - 1, col + 2, 1):
                if (
                    (playerX - (iCol * 16) < 16)
                    and (playerX - (iCol * 16) > -16)
                    and (playerY - (iRow * 16) < 16)
                    and (playerY - (iRow * 16) > -16)
                ):
                    # check the offending tile ID
    ...
    def Restart(self):
        for i in range(0, 4, 1):
            ...
            while not self.GetMapTile(randRow, randCol) == tileID["pellet"] or (
                randRow,
                randCol,
            ) == (0, 0):
            ...
            ghosts[i].currentPath = path.FindPath(
                (ghosts[i].nearestRow, ghosts[i].nearestCol), (randRow, randCol)
            )
            ghosts[i].FollowNextPathWay()
...
window = pygame.display.set_mode(
    thisGame.screenSize, pygame.DOUBLEBUF | pygame.HWSURFACE
)
    ...
    if not thisGame.mode == 8:
        ...
        if thisGame.fruitScoreTimer > 0:
            if thisGame.modeTimer % 2 == 0:
                thisGame.DrawNumber(
                    2500,
                    thisFruit.x - thisGame.screenPixelPos[0] - 16,
