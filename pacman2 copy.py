...
class path_finder:
    ...
    def CalcH(self, val):
        row, col = val
        self.map[self.Unfold(row, col)].h = math.sqrt(
            abs(row - self.end[0]) * abs(row - self.end[0])
            + abs(col - self.end[0]) * abs(col - self.end[0])
        )
...
def PressToStart(events):
    for event in events:
        if event.type == pygame.KEYDOWN:
            if event.key == pygame.K_SPACE and thisGame.mode == 1:
                thisGame.SetMode(2)
